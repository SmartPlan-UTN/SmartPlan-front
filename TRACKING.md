...
| CU | Feature | Screen | Status | Branch | PR |
|---|---|---|---|---|---|
| CU5 | Edit profile | PAN 14 | `Completed` | `SMART-18-cu6-cambiar-contrasena` | #92 |
| CU6 | Change password | PAN 14 | `Completed` | `SMART-18-cu6-cambiar-contrasena` | #92 |
| CU7 | Delete account | PAN 14 | `Completed` | `SMART-19-cu7-eliminar-cuenta` | #112 |
...
| CU | Feature | Screen | Status | Branch | PR |
|---|---|---|---|---|---|
| CU44 | Rate activity | PAN 18 | `In review` | `feature/cu44-cu45-cu46-cu47-valoraciones` | #113 |
| CU45 | View ratings | PAN 18 | `In review` | `feature/cu44-cu45-cu46-cu47-valoraciones` | #113 |
| CU46 | Edit rating | — | `In review` | `feature/cu44-cu45-cu46-cu47-valoraciones` | #113 |
| CU47 | Delete rating | — | `In review` | `feature/cu44-cu45-cu46-cu47-valoraciones` | #113 |
...
| CU60 | Manage plans | PAN 22 | `In review` | `SMART-59-63-cu53-cu60-admin-management` | #109 |
...
| Date | What happened |
|---|---|
| 2026-08-31 | CU53 and CU60 moved to review together in PR #109. PAN 21 manages activities, optional categories, and zero or more registered places without exposing Maps coordinates/provider metadata; PAN 22 manages plan search, status filtering, editing, and logical deletion. The complete `/admin` layout now requires the administrator role. Desktop/mobile and keyboard flows were smoke-tested with Playwright. `pnpm lint`, `pnpm test` (159), and `pnpm build` pass. Depends on SmartPlan-back#84. |
...
| 2026-09-01 | Opened `feature/cu44-cu45-cu46-cu47-valoraciones` off `develop` for the whole Valoraciones module and started CU44 (rate an activity, PAN 18, issue #55). `SmartPlan-back`'s ratings module (`GET`/`POST /activities/:id/ratings`, `GET .../ratings/me`) was already implemented and tested — no backend work needed. `ActivityDetail.jsx` has no write-a-rating control at all (only a read-only "Valoraciones" tab), so `RatingForm` (score via new `StarRatingInput` primitive + optional comment, `activity.module.css`) and its orchestrator `ActivityRatingSection` (auth gate, own-rating check, plan-eligibility check) are original, following `CreatePlanForm`'s hand-rolled label/input pattern rather than `AuthField`. `CreateRatingDto.planId` requires a completed plan of the user's that included the activity (`ratings.service.ts`'s `requireEligiblePlan`); no plan can reach `completed` through any code path in either repo yet (only ever read, never written), so `findEligiblePlanId` resolves it silently against the user's own plans (`listOwnPlans` + `getOwnPlan`, both already existing) with no picker UI, and the form stays hidden behind an explanatory message until that's possible — confirmed by the user before building it this way. Rewrote the stale `types/ratings.ts` (`idFeedback`/no `planId`) to match the real `OwnRatingDto` contract. `pnpm lint`, `pnpm test` (160), and `pnpm build` green. |
...
| 2026-09-01 | CU45: view ratings (PAN 18, issue #56). `RatingsList` (new, `components/activity/`) replaces the Valoraciones tab's static "Todavía no hay reseñas" placeholder with the real, paginated `GET /activities/:id/ratings` list — reuses `Pagination` (`components/explore`, already the app's "no design reference, follow the design system" prev/next control for CU9/CU12) instead of the mockup's own "ver todas" button, since that only ever reveals three hardcoded objects already in memory and the issue explicitly asks for real pagination. Each card matches `ActivityDetail.jsx`'s `ReviewCard` (avatar, author alias, relative date via the existing `formatRelativeTime`, stars, comment) except the avatar: one consistent ember gradient (`ProfileForm`'s own avatar treatment) instead of a color per reviewer, since the mockup's `review.color` has no real per-user data behind it. Left out, deliberately and not in the issue's scope either: the star-distribution bars (`RatingSummaryDto` has no per-star breakdown) and the "valoraciones externas" section (no external-ratings integration exists). Verified against the real running backend: approved a pending test rating in the dev DB and confirmed `GET /activities/1/ratings` returns it with the exact `{data, pagination, summary}` shape the frontend expects. `pnpm lint`, `pnpm test` (164), and `pnpm build` green. |
| 2026-09-01 | CU46 (issue #57) and CU47 (issue #58): edit and delete an own rating. `SmartPlan-back`'s `PATCH`/`DELETE /ratings/:id` were already implemented, tested, and seeded into `BOTH_ROLES` (`rating.update`/`rating.delete`) — no backend work needed. Like CU44, `ActivityDetail.jsx` has no edit/delete-own-review UI at all, so both are original: pencil/trash icon buttons on `ActivityRatingSection`'s "Tu valoración" card (matching `CollectionDetailView`'s existing neutral/hover-danger icon-button treatment) open, respectively, a new `EditRatingForm` — pre-filled, a standalone component rather than a `RatingForm` mode-switch, following this codebase's existing `CreatePlanForm`/`EditPlanForm` precedent of separate create/edit components — or the shared `ConfirmationDialog` (the same destructive-action pattern CU26/CU33/CU34 already use). "Solo el autor puede editarla" (CU46) needs no extra frontend check: this card only ever renders for the signed-in user's own rating, and the backend re-scopes by `idUser` regardless. CU47's "Recalculo del promedio" is handled by treating `SmartPlan-back` as the only source of truth rather than computing it client-side: `ActivityRatingSection` now takes an `onChange` callback (fired after any create/edit/delete, tying CU44 into the same mechanism) that bumps a `refreshToken` on `RatingsList`, which resets to page 1 and refetches, handing its freshly-fetched `summary` back up to replace `ActivityDetailView`'s initial, potentially-stale `activity.averageRating`/`ratingCount` snapshot (shown in both the Valoraciones tab and the title-block badge). Verified against the real running backend end-to-end on the existing test rating: edited it (score/comment), confirmed the edited comment re-entered moderation as `pending` — dropping it out of the public list and the average, correctly reflected by the refresh mechanism — restored it to `approved`, then deleted it (`204`), confirmed the average/count recalculated to 0, `GET .../ratings/me` returned empty, and a second delete correctly 404s `RATING_NOT_FOUND`. `pnpm lint`, `pnpm test` (177), and `pnpm build` green. |
| 2026-09-01 | Opened PRs #92 (CU5/CU6, `SMART-18-cu6-cambiar-contrasena`), #112 (CU7, `SMART-19-cu7-eliminar-cuenta`), #113 (CU44-CU47, this branch), and back#86 (`feature/sessions-me-endpoint`). Closed #90 (`SMART-16-cu4-cerrar-sesion`) and #91 (`SMART-17-cu5-editar-perfil`) as superseded: both were older, separately-authored, still-open PRs for CU4/CU5 whose ground #92 independently reimplements with more scope (the logout-popup portal fix `SMART-16` doesn't have, plus CU6/CU7/Security). #92 and #112 merged first; merging `develop` back into this branch afterward hit a single conflict in this file's own log (both sides appended entries after the same line) — resolved by interleaving both sides' entries in chronological order, no code conflict. |
