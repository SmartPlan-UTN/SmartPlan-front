...
| CU12 | Search plans | PAN 10, PAN 11 | `Completed` | `SMART-21-cu9-buscar-actividades` | [#87](https://github.com/SmartPlan-UTN/SmartPlan-front/pull/87) |
| CU13 | View plan | PAN 17 | `Completed` | `SMART-21-cu9-buscar-actividades` | [#87](https://github.com/SmartPlan-UTN/SmartPlan-front/pull/87) |
...
| 2026-09-02 | CU12/CU13 visual audit against `Results.jsx`/`PlanDetail.jsx`, requested with side-by-side screenshots: corrected the CU12/CU13 status rows above to `Completed` (PR #87 merged long ago, the rows had just never been updated). Three fixes: (1) `PlanSearch`'s first-load state now matches `ResultsLoading` — a big, centered "Armando tu plan perfecto..." transition with the query quoted underneath — instead of the small inline `LoadingDots` every other list uses; `LoadingDots` gained an optional `title` prop for this (backward compatible, every other caller still just passes `label`). (2) `PlanCard` gained the mockup's "Ver plan completo" CTA as a decorative pill (the click affordance stays the card-wide `Link`, same reasoning as `PlanDetailView`'s `.stepDetailHint`) — the "Mejor match" badge on the mockup's top pick was left out, since it needs a relevance signal `PlanSearchResult` doesn't have and nothing here approximates one. (3) `PlanDetailView` gets back the mockup's 300px dark hero with the title/rating/duration/location overlaid on it (an earlier pass had flattened this to a content-sized header on the cream background instead); `FloatingBackLink` now tracks it via `heroRef`, same as `ActivityDetailView`. Neither the card banner nor the hero uses the mockup's per-plan emoji: confirmed against the user first (`skills/06-design-system/SKILL.md` — "No emoji in the product interface" — the same rule `ActivityCard`/`ActivityDetailView` already followed), so both use a muted `route` icon watermark instead, matching those two exactly. `pnpm lint`, `pnpm test` (216), and `pnpm build` green. |
