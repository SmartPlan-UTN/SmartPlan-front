...
| Date | What happened |
|---|---|
| 2026-08-25 | The app-wide wave background moved to review in PR #98 on `feature/fondo-olas`. `AppShell` mounts one `MoodBackground` for the whole shell so the water keeps its phase across navigation, `section-mood.ts` maps each route to a palette, and every navigation breaks a wave. Rendering moved off the main thread into a Web Worker drawing to an `OffscreenCanvas`, which is what stopped the water freezing during a route change; the swell became a travelling Gerstner packet so it reads as water rather than as the amplitude being turned up. No CU: cross-cutting UI, no associated issue. `pnpm lint`, `pnpm test` (121), and `pnpm build` pass. |
