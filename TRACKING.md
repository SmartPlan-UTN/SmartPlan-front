...
| 2026-09-01 | Re-audited CU6 against the v2 system design: `Profile.jsx` duplicates the password-change card inline, but the design also has a dedicated standalone `Security.jsx` screen for it that was never wired up. Moved `ChangePasswordForm` off `/profile` onto its own `/security` destination (`SecurityScreen`, new `components/security/`), added "Seguridad" as a fourth entry in the user menu between Preferencias and Cerrar sesión, and gave the form the mockup's decorative top strip, section icon/header, strength bar, and full three-row requirements checklist (still only enforcing the real length rule — see the doc comment on `ChangePasswordForm` for why the other two rows stay informational). `ProfileScreen` keeps only `ProfileForm` now. Also fixed a real, independently-reproducible bug found in the same pass: `UserMenu`'s `LogoutConfirmModal` rendered inside `Navbar`'s `<header>`, which sets `backdrop-filter` for its sticky-blur effect — `backdrop-filter` establishes a containing block for `position: fixed` descendants, so the modal was confined to the header's box (a small card pinned near the top) instead of covering the viewport. Portaled it to `document.body`, same fix pattern the new `DeleteAccountModal`-style dialogs already use. Also applied the `.backdrop`/`Container`-padding breakout fix (already used by `security.module.css`) to `profile.module.css` too, so `MoodBackground`'s waves reach the container's real edges on both screens instead of leaving cream gaps above the heading and below the last card. The prototype's "Sesiones activas" footer card on `Security.jsx` is still deliberately left out — no backend endpoint to list or revoke a subset of sessions exists yet. `pnpm lint`, `pnpm test` (173), and `pnpm build` green. |
