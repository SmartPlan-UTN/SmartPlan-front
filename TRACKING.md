...
| CU8 | Edit preferences | PAN 15 | `In review` | `SMART-20-cu8-editar-preferencias` | #20 |
...
| 2026-08-28 | CU8/CU18: implemented preference editing (PAN 15), matching the v2 system design's `Preferences.jsx`. `PreferencesForm` (`src/components/preferences/`) loads and saves the full profile in one `PATCH /users/me/preferences`: interest chips (reusing CU10's `Chip`, real category names — no emoji, matching that same brand-voice deviation from the mockup), usual budget (client-side error only for an actually-invalid value, not an untouched empty field — the mockup's unconditional error is a demo artifact, not the real rule), usual party size (1-20 counter), max distance (1-50km slider with presets), and preferred area. Preferred area resolves through Google's classic `AutocompleteService`/`PlacesService` (new `src/lib/maps/placeSearch.ts`, `loadGoogleMaps.ts` now also requests `libraries=places`) or the device's GPS reverse-geocoded the same way, since `preferred_area`'s four backend columns move together — free text the user hasn't picked a suggestion for never gets sent. New `Toggle` primitive (`components/ui/`) for "usar ubicación del dispositivo", ported from the prototype's track/thumb switch. `PreferencesForm` takes an `onSaved` callback so a future registration step (CU8's "reutilizable desde el registro") can hook into a successful save — not wired into CU2's signup in this PR, which has no such step designed yet. Replaced the `PendingScreen` placeholder at `/preferences`; `AppShell`'s already-global wave background covers it (`idle`, unlisted in `section-mood.ts`), so `PreferencesScreen` only needed the `.backdrop`/`.wrapper` split `explore` already established. `pnpm lint`, `pnpm test` (161), and `pnpm build` green. |
