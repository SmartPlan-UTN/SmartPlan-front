...
| 2026-08-25 | Logout confirmation uses `Button variant="primary"` for "Confirmar", not `variant="danger"` | Verified directly against `SmartPlanSystemDesign-v2/v2/Navbar.jsx`'s `LogoutModal`: `.sp-modal-btn.confirm` is ember (`background: var(--ember)`), with a matching ember box-shadow. Only the icon above the title is tinted with the destructive color — the prototype treats logging out as a normal action, not one that needs a red warning button the way deleting an account would |
| 2026-08-25 | Added `--danger` (`#D94B2B`) and `--danger-10` to `tokens.css` | The v2 prototype uses this exact literal — distinct from `--error`'s `#F04040` — for the LogoutModal's icon and Profile's "Eliminar cuenta" danger zone (CU7). Same reasoning as `--rating-ink`/`--warning-ink`: a color the base EMBER v2 tokens don't define but a designed screen actually needs, added once ported rather than left as a hand-written hex |
