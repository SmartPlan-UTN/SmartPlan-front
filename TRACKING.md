...
|---|---|---|---|---|---|
| CU1 | Log in | PAN 04 | `Completed` | `SMART-13-cu1-iniciar-sesion` | #88 |
| CU2 | Register user | PAN 04 | `Completed` | `SMART-13-cu1-iniciar-sesion` | #88 |
| CU3 | Recover password | PAN 05 | `In progress` | `SMART-15-cu3-recuperar-contrasena` | |
...
| 2026-08-24 | CU3's password-recovery screens moved onto `AuthSplitShell`, retiring the old dark `(auth)/` route group entirely | Login/signup already moved there; the v2 prototype nests the recovery-request form as a third view inside that same card (`v2/Login.jsx`'s `ForgotPasswordForm`), not the old dark shell. Once recovery moved, `(auth)/` had no screens left in it and was deleted (404 keeps using the same `.centeredScreen`/`.centeredCard` classes directly from `layout.module.css`) |
| 2026-08-24 | An invalid, expired, or already-used recovery token replaces the reset-password form with a dedicated view, instead of a form-level error banner | Resubmitting can only fail again — the token itself is what's wrong, not what the user typed. The replacement view's CTA points at `/recover-password` instead |
| 2026-08-24 | `EMAIL_NOT_REGISTERED` on the recovery-request form is shown as a field error, not a form banner | It's specific, actionable feedback about the address typed, consistent with how every other per-field backend error is already surfaced in `LoginForm`/`RegisterForm` |
...
| 2026-08-24 | CU3: implemented password recovery (PAN 05) — `RecoverPasswordForm` (request an email, `POST /password-recoveries`) and `ResetPasswordForm` (set a new password from the emailed link's token, `PATCH /password-recoveries`), both added to `src/lib/auth/api.ts`. Both screens moved onto `AuthSplitShell`, the same split-screen shell CU1/CU2 use, following the v2 prototype's `ForgotPasswordForm`; the now-empty `(auth)/` route group was deleted. New `/reset-password` route reads `token` from the query string (the link SmartPlan-back's email sends: `${FRONTEND_URL}/reset-password?token=...`); a missing or rejected token (`INVALID_RECOVERY_TOKEN`, `EXPIRED_RECOVERY_TOKEN`, `RECOVERY_TOKEN_ALREADY_USED`) replaces the form with a broken-link view offering a new request instead of a form error, since resubmitting the same form can't succeed. Verified the exact contract (routes, DTOs, error codes, the `${FRONTEND_URL}/reset-password?token=` link) by reading `SmartPlan-back`'s `origin/develop` directly, same as CU1 — this time the shared integration doc matched the real code exactly. Added a `mail-check` icon. `pnpm lint`, `pnpm test` (47), and `pnpm build` green. |
...
| 2026-08-25 | Merged the latest `develop` (CU9-CU16 exploration/map work) into this CU3 branch to clear a merge-conflict block reported on the PR. Only `TRACKING.md` and `skills/06-design-system/SKILL.md` conflicted — both documentation, resolved by keeping both sides' entries; every code file merged automatically. |
| 2026-08-25 | Visual audit against `SmartPlanSystemDesign-v2/v2/Login.jsx`'s `ForgotPasswordForm` surfaced and fixed two real deviations. (1) The step-bar segments both grew to 20px/ember at step 2: the prototype fixes each segment's *width* by position (`i === 1 ? 20 : 8`, always) and only changes *background* with progress — mine coupled both to the same "active" class, so step 2 rendered two wide bars instead of one wide + one narrow, both ember. Split into `.stepSegmentFirst` (width) and `.stepSegmentActive` (background). (2) "Reenviar" used `variant="ghostEmber"` (filled ember-15 background); the prototype's is transparent with a 1.5px ember border. Switched to `variant="ghostLight"` (already transparent) with a local `button.resendButton` override for the border/text color — element-qualified so it doesn't depend on which CSS Module's rule the bundler happens to emit later. Verified both fixes in the compiled CSS output, not just the source. |
...
| 2026-08-25 | Merged the latest `develop` (CU32-CU34 collections work, F06's CI alignment) into this CU3 branch to clear the merge-conflict block GitHub reported on the PR after `develop` moved again. Only `TRACKING.md` conflicted, same as the previous round — resolved by keeping both sides' log entries. |
